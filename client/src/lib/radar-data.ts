import type { RadarModel, Nation, RadarResponse, RadarQuery } from "@shared/schema";
...
function calculateScore(
  ...
  momPct: number,
  rankChange: number,
  allModels: { momAbs: number; momPct: number; rankChange: number }[]
): number {
  ...
  return 0.55 * zMomAbs + 0.35 * zMomPct + 0.1 * zRankChange;
}

type RawModel = Omit<RadarModel, "id" | "score" | "danawaUrl">;

const domesticModels2025_12: RawModel[] = [
...
const domesticModels2025_11: RawModel[] = [
...
const exportModels2025_12: RawModel[] = [
...
const exportModels2025_11: RawModel[] = [
...
function initializeData(): RadarModel[] {
  const dataWithScores: RadarModel[] = [];

  const groupedData: Record<string, RawModel[]> = {};
  for (const model of allRawData) {
...
const radarData = initializeData();
const lastUpdated = new Date().toISOString();

export function getAvailableMonths(): string[] {
  const months = new Set<string>();
  for (const model of radarData) {
    months.add(model.month);
  }
  return Array.from(months).sort().reverse();
}

export function getRadarData(query: RadarQuery): RadarResponse {
